...
(function () {
    ...
    WordSearch.prototype.parseWords = function (maxSize) {
        ...
        for (var i = 0; i < this.settings.words.length; i++) {
            ...
            this.settings.wordsList[i] = this.settings.words[i].phrase.trim();
            this.settings.words[i].phrase = removeDiacritics(this.settings.wordsList[i].trim().toUpperCase());

            var word = this.settings.words[i].phrase;
    ...
    WordSearch.prototype.addWords = function () {
        ...
        while (keepGoing) {
            ...
            var dir = this.settings.directions[Math.rangeInt(this.settings.directions.length - 1)],
                result = this.addWord(this.settings.words[counter].phrase, dir),
                isWorked = true;
    ...
    WordSearch.prototype.fillUpFools = function () {
        var rangeLanguage = searchLanguage(this.settings.words[0].phrase.split('')[0]);
        for (var row = 0; row < this.settings.gridSize; row++) {
            for (var col = 0; col < this.settings.gridSize; col++) {
    ...
    WordSearch.prototype.lookup = function (selected) {
        ...
        if (this.settings.words.findIndex(word => word.phrase === words[0]) > -1 ||
            ...
            for (var i = 0; i < wordListItems.length; i++) {
                const currWord = this.settings.words[i];
...
window.anxietyInstance = new Anxiety();
window.anxietyInstance.init();